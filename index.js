const { Client, Intents, MessageButton, DiscordAPIError, MessageSelectMenu, MessageAttachment, MessageEmbed, MessageActionRow } = require('discord.js');
...
client.once("ready", async () => {
  ...
  client.on("voiceStateUpdate", async (oldState, newState) => {
    ...
    if (newChannel && newChannel.id === channelIdToJoin) {
      ...
      if (textChannel) {
        textChannel.send(
          ` : ${newState.member.user} في انتظار الدعم الفني.`,
        );
      }
      ...
      // Inside the voiceStateUpdate listener
      setTimeout(async () => {
        const connection = await joinVoiceChannelAndPlay();
        if (connection) {
          const player = createAudioPlayer();
          connection.subscribe(player);
          // Change the audio file URL here
          const resource = createAudioResource('./assets/support.mp3');
          player.play(resource);
          isPlaying = true;
          // Play the second audio file after the first one ends
          player.on(AudioPlayerStatus.Idle, () => {
            const resource2 = createAudioResource('https://qurango.net/radio/tarateel');
